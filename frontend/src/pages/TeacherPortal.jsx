...
function CGPAComponent({ usn }) {
  ...
  useEffect(() => {
    const fetchCGPA = async () => {
      if (!usn) return;
      try {
        const { data, error } = await supabase.rpc("calculate_cgpa", { p_usn: usn });
...
export default function TeacherPortal() {
  ...
  useEffect(() => {
    const fetchGroupStudents = () => {
      ...
      const group = createdGroups.find(g => g.Group_ID === selectedGroup);
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      ...
      try {
        ...
        const { data: academicData, error: academicError } = await supabase.rpc('get_academic_details_by_usn', { p_usn: selectedStudent });
        if (academicError) throw academicError;
        setAcademicRecords(academicData || []);

        // Fetch skills
        const { data: skillsData, error: skillsError } = await supabase.rpc('get_student_skills', { p_usn: selectedStudent });
    ...
    try {
      ...
      const studentsWithCGPA = await Promise.all(
        group.students.map(async (student) => {
          try {
            const { data: cgpa, error } = await supabase.rpc("calculate_cgpa", { p_usn: student.usn });
            if (error) throw error;
            return {
              usn: student.usn,
            ...
            console.error(`Error fetching CGPA for ${student.usn}:`, err);
            return {
              usn: student.usn,
              name: `${student.first_name} ${student.last_name}`,
              cgpa: 0,
  ...
  const calculateCGPADistribution = (students) => {
    ...
    return ranges.map(range => {
      const count = students.filter(s => s.cgpa >= range.min && s.cgpa < range.max).length;
      const percentage = students.length > 0 ? (count / students.length * 100).toFixed(1) : "0.0";
  ...
  const handleGroupSubmit = async (e) => {
    ...
    try {
      const adminID =
        teacherData?.Admin_ID || setupForm.Admin_ID || "X";
    ...
    <Box sx={{ 
      ...
      >
        ...
        {teacherData?.Admin_ID && (
          <Typography variant="body2" sx={{ opacity: 0.9, mb: 0.5, color: "rgba(255,255,255,0.9)" }}>
            ID: {teacherData.Admin_ID}
          </Typography>
        )}
        {teacherData?.Designation && (
          <Typography variant="body2" sx={{ opacity: 0.9, color: "rgba(255,255,255,0.9)" }}>
            {teacherData.Designation}
          </Typography>
        )}
      </Box>
        ...
        {menuItems.map((item) => (
          <ListItem key={item.value} disablePadding sx={{ mb: 0.5, px: 1 }}>
            ...
            >
              <ListItemIcon
                sx={{
                  color: activeTab === item.value ? "#f093fb" : "rgba(255,255,255,0.6)",
                  minWidth: 40,
                }}
              >
                {item.icon}
              ...
              <Box>
                ...
                <form onSubmit={handleSetupSubmit}>
                  ...
                  >
                    ...
                    >
                      <PersonIcon sx={{ mr: 1, color: "#f093fb", fontSize: 28 }} />
                      <Typography variant="h6" sx={{ 
                        fontWeight: 600,
                        color: "rgba(255, 255, 255, 0.95)",
                      }}>
                    ...
                    <Grid container spacing={3}>
                      <Grid item xs={12} sm={4}>
                        <TextField
                          ...
                          onChange={(e) =>
                            setSetupForm({ ...setupForm, Admin_ID: e.target.value })
                          }
                          required
                      ...
                      <Grid item xs={12} sm={4}>
                        <TextField
                          ...
                          InputProps={{
                            startAdornment: (
                              <InputAdornment position="start">
                                <WorkIcon sx={{ color: "rgba(255, 255, 255, 0.5)" }} />
                              </InputAdornment>
                            ),
                          }}
                          sx={{
                          ...
                          <Button
                            ...
                            onClick={() =>
                              setSetupForm({
                                ...
                                Phone: teacherData?.Phone_Number || teacherData?.Phone || "",
                ...
                {groupsLoading ? (
                  ...
                  >
                    <Box sx={{ 
                      ...
                      <GroupIcon sx={{ fontSize: 40, color: "rgba(240, 147, 251, 0.5)" }} />
                    </Box>
                    ...
                    <Typography variant="body1" sx={{ 
                      ...
                      Go to "Manage Groups" to create your first student group.
                    </Typography>
                    <Button
                      variant="contained"
                  ...
                  <Grid container spacing={3}>
                    {createdGroups.map((group, index) => {
                      const groupId = group.Group_ID || `group-${index}`;
                      const groupName = group.Group_Name || "Unnamed Group";
                      const currentYear = group.Current_Year || "N/A";
                      const studentCount = group.studentCount || 0;
                      const students = group.students || [];

                      return (
                        <Grid item xs={12} md={6} key={groupId}>
                          <Paper
                            elevation={0}
                            sx={{
                              p: 3,
                          ...
                          >
                            ...
                            {groupId && (
                              ...
                              >
                                Group ID: <strong style={{ color: "rgba(255, 255, 255, 0.9)" }}>{groupId}</strong>
                              </Typography>
                            )}
                            ...
                            >
                              <PeopleIcon fontSize="small" sx={{ color: "rgba(255, 255, 255, 0.6)" }} />
                              <Typography variant="body2" sx={{ 
                                color: "rgba(255, 255, 255, 0.6)",
                              }}>
                ...
                <form onSubmit={handleGroupSubmit}>
                  ...
                  >
                    <Box
                      sx={{ display: "flex", alignItems: "center", mb: 3 }}
                    >
                      <GroupIcon sx={{ mr: 1, color: "#f093fb", fontSize: 28 }} />
                      <Typography variant="h6" sx={{ 
                        fontWeight: 600,
                        color: "rgba(255, 255, 255, 0.95)",
                      }}>
                    ...
                    <Grid container spacing={3}>
                      <Grid item xs={12} sm={4}>
                        <TextField
                          ...
                          onChange={(e) =>
                            setGroupData({ ...groupData, groupID: e.target.value })
                          }
                          required
                          sx={{
                            "& .MuiOutlinedInput-root": {
                      ...
                      <Grid item xs={12}>
                        <TextField
                          ...
                          InputProps={{
                            startAdornment: (
                              <InputAdornment position="start">
                                <SchoolIcon sx={{ color: "rgba(255, 255, 255, 0.5)" }} />
                              </InputAdornment>
                            ),
                          }}
                          sx={{
                        ...
                        >
                          {studentOptions.map((s) => (
                            <MenuItem key={s.usn} value={s.usn} sx={{
                              bgcolor: "#1a1a2e",
                              color: "rgba(255, 255, 255, 0.9)",
                              "&:hover": {
                                bgcolor: "rgba(240, 147, 251, 0.2)",
                              ...
                              "&.Mui-selected": {
                                bgcolor: "rgba(240, 147, 251, 0.3)",
                                "&:hover": {
                                  bgcolor: "rgba(240, 147, 251, 0.4)",
                                },
                              },
                            }}>
                              {s.first_name} {s.last_name} ({s.usn})
                            </MenuItem>
                          ))}
                        </TextField>
                      </Grid>
            ...
            {activeTab === "analysis" && (
              <Box>
                ...
                <Typography variant="body2" sx={{ 
                  ...
                  Analyze student performance, skills, and compare within groups
                </Typography>
                    ...
                    <Grid item xs={12} md={6}>
                      ...
                      >
                        ...
                        {createdGroups.map((group) => (
                          <MenuItem key={group.Group_ID} value={group.Group_ID} sx={{
                            bgcolor: "#1a1a2e",
                            color: "rgba(255, 255, 255, 0.9)",
                            "&:hover": {
                              bgcolor: "rgba(240, 147, 251, 0.2)",
                            ...
                            {group.Group_Name} ({group.studentCount} students)
                          </MenuItem>
                        ))}
                      </TextField>
                    </Grid>
                    ...
                    <Grid item xs={12} md={6}>
                      ...
                      >
                        ...
                        {groupStudents.map((student) => (
                          <MenuItem key={student.usn} value={student.usn} sx={{
                            bgcolor: "#1a1a2e",
                            color: "rgba(255, 255, 255, 0.9)",
                            "&:hover": {
                              bgcolor: "rgba(240, 147, 251, 0.2)",
                  ...
                  <>
                      ...
                      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 3 }}>
                        <Box>
                          <Typography variant="h5" sx={{ 
                            ...
                            {groupStudents.find(s => s.usn === selectedStudent)?.first_name} {groupStudents.find(s => s.usn === selectedStudent)?.last_name}
                          </Typography>
                          <Typography variant="body2" sx={{ 
                            color: "rgba(255, 255, 255, 0.6)",
                          }}>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                          ...
                          <Typography variant="h4" sx={{ 
                            ...
                            <CGPAComponent usn={selectedStudent} />
                          </Typography>
                        </Box>
                      </Box>
                    ...
                    >
                      <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
                        <SchoolIcon sx={{ mr: 1, color: "#f093fb", fontSize: 28 }} />
                        <Typography variant="h6" sx={{ 
                          fontWeight: 600,
                          color: "rgba(255, 255, 255, 0.95)",
                        }}>
                      ...
                      {academicRecords.length > 0 ? (
                        <TableContainer>
                          <Table>
                            <TableHead>
                              <TableRow>
                                <TableCell sx={{ color: "rgba(255, 255, 255, 0.9)", fontWeight: 600 }}>Course Code</TableCell>
                                <TableCell sx={{ color: "rgba(255, 255, 255, 0.9)", fontWeight: 600 }}>Course Name</TableCell>
                                <TableCell sx={{ color: "rgba(255, 255, 255, 0.9)", fontWeight: 600 }}>Semester</TableCell>
                                <TableCell sx={{ color: "rgba(255, 255, 255, 0.9)", fontWeight: 600 }}>Year</TableCell>
                                <TableCell sx={{ color: "rgba(255, 255, 255, 0.9)", fontWeight: 600 }}>Grade</TableCell>
                                <TableCell sx={{ color: "rgba(255, 255, 255, 0.9)", fontWeight: 600 }}>Credits</TableCell>
                              </TableRow>
                            </TableHead>
                            <TableBody>
                              {academicRecords.map((record, index) => (
                                <TableRow key={index}>
                                  <TableCell sx={{ color: "rgba(255, 255, 255, 0.8)" }}>{record.Course_Code}</TableCell>
                                  <TableCell sx={{ color: "rgba(255, 255, 255, 0.8)" }}>{record.Course_Name}</TableCell>
                                  <TableCell sx={{ color: "rgba(255, 255, 255, 0.8)" }}>{record.Semester}</TableCell>
                                  <TableCell sx={{ color: "rgba(255, 255, 255, 0.8)" }}>{record.Year}</TableCell>
                                  <TableCell>
                                    <Chip
                                      label={record.Grade}
                                      size="small"
                                  ...
                                  <TableCell sx={{ color: "rgba(255, 255, 255, 0.8)" }}>{record.Credits_earned}</TableCell>
                                </TableRow>
                              ))}
                            </TableBody>
                          </Table>
                    ...
                    >
                      ...
                      {skills.length > 0 ? (
                        <Grid container spacing={2}>
                          {skills.map((skill) => (
                            <Grid item xs={12} sm={6} md={4} key={skill["Sl.No"]}>
                              <Card variant="outlined" sx={{ 
                                p: 2,
                                bgcolor: "rgba(255, 255, 255, 0.03)",
                                borderColor: "rgba(255, 255, 255, 0.08)",
                                ...
                                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
                                  ...
                                  <Chip
                                    ...
                                    size="small"
                                    sx={{ 
                                      bgcolor: getRatingColor(skill.Rating),
                                      color: "white",
                                      fontWeight: 600,
                                    }}
                                  />
                    ...
                    >
                      <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
                        <CompareIcon sx={{ mr: 1, color: "#FF9800", fontSize: 28 }} />
                        <Typography variant="h6" sx={{ 
                          fontWeight: 600,
                          color: "rgba(255, 255, 255, 0.95)",
                        }}>
                          CGPA Comparison ({studentCGPAComparison.length} students)
                        </Typography>
                      </Box>

                      {studentCGPAComparison.length > 0 ? (
                        <>
                          {/* CGPA Bar Chart */}
                          <Box sx={{ mb: 4 }}>
                            <Typography variant="h6" sx={{ 
                            ...
                            <Box sx={{ display: "flex", justifyContent: "center" }}>
                              <ResponsiveContainer width="100%" height={300}>
                                <BarChart
                                  data={studentCGPAComparison.map((student, index) => ({
                                    name: student.name.length > 10 ? student.name.substring(0, 10) + '...' : student.name,
                                    cgpa: student.cgpa,
                                    fill: student.usn === selectedStudent ? '#f093fb' : '#2196F3'
                                  }))}
                                  margin={{
                                    top: 5,
                                    right: 30,
                                ...
                                >
                                  ...
                                  <Tooltip 
                                    contentStyle={{ backgroundColor: 'rgba(26,26,46,0.9)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px' }}
                                    labelStyle={{ color: 'white' }}
                                  />
                                  <Bar dataKey="cgpa" fill="#2196F3" />
                                </BarChart>
                              </ResponsiveContainer>
                            </Box>
                          </Box>
                        ...
                        <Box sx={{ textAlign: "center", py: 4 }}>
                          <Typography variant="body1" sx={{ color: "rgba(255, 255, 255, 0.6)" }}>
                            No CGPA data available for comparison.
                          </Typography>
                        </Box>
                      )}
                    </Paper>
                ...
                {!selectedStudent && !analysisLoading && (
                  ...
                  >
                    <Box sx={{ 
                      ...
                      <AnalyticsIcon sx={{ fontSize: 40, color: "rgba(240, 147, 251, 0.5)" }} />
                    </Box>
                    <Typography variant="h5" sx={{ 
                      fontWeight: 700, 
                      color: "rgba(255, 255, 255, 0.95)", 
