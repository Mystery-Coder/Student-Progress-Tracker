...
export default function AddAcademicForm({ student }) {
	...
	// NEW: Track selected courses in localStorage
	...
	// NEW: Save selected courses to localStorage whenever they change
	useEffect(() => {
		try {
			localStorage.setItem(
				"selectedCourses",
				JSON.stringify(selectedCourseCodes),
			);
		} catch (error) {
	...
	// NEW: Function to check if a course is already selected
	const isCourseSelected = (courseCode) => {
		return selectedCourseCodes.includes(courseCode);
	};

	// NEW: Function to add a course to selected list
	const addSelectedCourse = (courseCode) => {
		if (!isCourseSelected(courseCode)) {
			setSelectedCourseCodes((prev) => [...prev, courseCode]);
		}
	};
		...
		const fetchAcademicRecords = async () => {
			...
			try {
				...
				if (error) {
					...
					setMessage({
						type: "error",
						text: "Failed to load academic records",
					});
				} else {
	...
	const handleSubmit = async (e) => {
		...
		// NEW: Check if course is already selected
		if (isCourseSelected(formData.Course_Code)) {
			setMessage({
				type: "error",
				text: `Course "${formData.Course_Code}" has already been selected. You cannot select the same course again.`,
			});
			return;
		}
		...
		try {
			...
			// NEW: Add course to selected list after successful submission
			addSelectedCourse(formData.Course_Code);
			...
			if (activeSection === "records") {
				const { data: updatedRecords } = await supabase.rpc(
					"get_academic_details_by_usn",
					{
						p_usn: student.USN,
					},
	...
	const handleSkillsSubmit = async (e) => {
		...
		try {
			...
			const { data: updatedSkills, error: fetchError } =
	...
	const handleDeleteSkill = async (slNo) => {
		...
		try {
			...
			const { data, error } = await supabase.rpc("delete_student_skill", {
				p_sl_no: slNo,
				p_usn: student.USN,
			});
			...
			setSkills((prevSkills) =>
				prevSkills.filter((skill) => skill["Sl.No"] !== slNo),
			);
	...
	// NEW: Function to clear selected courses (for testing/debugging)
	const clearSelectedCourses = () => {
		if (
			window.confirm(
				"Clear all selected courses? This will allow you to select courses again.",
			)
		) {
			setSelectedCourseCodes([]);
			localStorage.removeItem("selectedCourses");
			setMessage({ type: "success", text: "Selected courses cleared!" });
		}
	};
	...
	return (
		...
		>
			...
			>
				Add your course grades, academic performance, and skills with
				ratings
			</Typography>

			{/* NEW: Selected courses info banner */}
			{activeSection === "academic" && selectedCourseCodes.length > 0 && (
				<Alert
					severity="info"
					sx={{
						mb: 3,
				...
				>
					<Typography variant="body2">
						You have already selected {selectedCourseCodes.length}{" "}
						course(s). Previously selected courses are disabled.
					</Typography>
				</Alert>
			)}
			...
			>
				<Button
					variant={
						activeSection === "academic" ? "contained" : "outlined"
					}
				...
				<Button
					variant={
						activeSection === "skills" ? "contained" : "outlined"
					}
				...
				<Button
					variant={
						activeSection === "records" ? "contained" : "outlined"
					}
			...
			{studentYear && activeSection === "academic" && (
				...
				>
					...
					Showing courses for Year {studentYear} (based on your
					assigned group)
				</Typography>
			)}
			{!studentYear && student?.USN && activeSection === "academic" && (
				...
				>
					<SchoolIcon sx={{ fontSize: 16 }} />
					You are not assigned to any group. Please contact your
					teacher to be assigned to a group.
				</Typography>
			)}
			...
			{activeSection === "academic" && (
				<>
					...
					>
						...
						>
							<AddCircleIcon
								sx={{ mr: 1, color: "#667eea", fontSize: 28 }}
							/>
							<Typography
								variant="h6"
								sx={{
					...
					{formData.Grade && (
						<Card
							elevation={0}
							sx={{
								bgcolor: isCourseSelected(formData.Course_Code)
									? "rgba(244, 67, 54, 0.1)"
									: "rgba(102, 126, 234, 0.1)",
								color: "rgba(255, 255, 255, 0.95)",
								borderRadius: 2,
								...
								mb: 3,
								border: `1px solid ${
									isCourseSelected(formData.Course_Code)
										? "rgba(244, 67, 54, 0.2)"
										: "rgba(102, 126, 234, 0.2)"
								}`,
							}}
						>
							<Typography variant="body2" sx={{ opacity: 0.9 }}>
								Preview: {formData.Course_Code} - Semester{" "}
								{formData.Semester} - Year {formData.Year} -
								Credits: {formData.Credits_Earned} - Grade{" "}
								<Chip
									label={formData.Grade}
									size="small"
									color={
										gradeColors[formData.Grade] || "default"
									}
			...
			{activeSection === "skills" && (
				<>
					...
					>
						...
						>
							<BuildIcon
								sx={{ mr: 1, color: "#4CAF50", fontSize: 28 }}
							/>
							<Typography
								variant="h6"
								sx={{
							...
							<Grid container spacing={3}>
								...
								<Grid item xs={12} md={2}>
									<Button
										...
										disabled={
											skillsLoading ||
											!skillsFormData.Skill_Name.trim()
										}
										sx={{
											height: "56px",
											background:
					...
					{skills.length > 0 ? (
						...
						>
							...
							>
								Add your first skill using the form above to get
								started.
							</Typography>
						</Paper>
					)}
			...
			{activeSection === "records" && (
				...
				>
					<Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
						<ListIcon
							sx={{ mr: 1, color: "#FF9800", fontSize: 28 }}
						/>
						<Typography
							variant="h6"
							sx={{
					...
					{recordsLoading ? (
						...
						<>
							...
							>
								<SchoolIcon sx={{ fontSize: 16 }} />
								Showing {academicRecords.length} academic
								record(s) for {student?.USN}
							</Typography>
						...
						>
							...
							>
								You haven't added any academic records yet. Go
								to the "Academic Records" tab to add your first
								course grade.
							</Typography>
							<Button
								variant="contained"
								...
								sx={{
									...
									"&:hover": {
										background:
											"linear-gradient(135deg, #5568d3 0%, #653a8e 100%)",
									},
								}}
							>
								<SchoolIcon sx={{ mr: 1 }} />
